...
from pyfm_fast import FM_fast_parallel, CSRDataset
...
class FM:
    ...
    def fit(self, X, y, dictVec):
        ...
        reg_speedup = self._bool_to_int(self.reg_speedup)
        ...
        attribute_list, attribute_feature = np.unique(map(lambda st : st.rsplit('=', 1)[0], dictVec.get_feature_names()), return_inverse=True)
        self.num_attribute = len(attribute_list)
        self.ind_feature_attribute = np.array(map(lambda x : float(x), attribute_feature))

        # Set up params
        self.w0 = 0.0
        self.w = np.zeros(self.num_feature)
        np.random.seed(seed=self.seed)
        self.v = np.random.normal(scale=self.init_stdev,size=(self.num_factors, self.num_feature))

        # Convert datasets to sklearn sequential datasets for fast traversal
        X_train_dataset = _make_dataset(X_train, train_labels)
        self.validation_dataset = _make_dataset(validation, validation_labels)

        self.fm_fast = FM_fast_parallel(self.w,
                               self.v,
                               self.num_factors,
                               self.num_feature,
                               self.num_attribute,
                               self.num_processes,
                               self.ind_feature_attribute,
                               self.num_iter,
                               self._bool_to_int(self.k0),
                               self._bool_to_int(self.k1),
                               self._bool_to_int(self.k2),
                               self.w0,
                               self.t,
                               self.t0,
                               self.power_t,
        ...
        return self.fm_fast.fit(X_train_dataset, self.validation_dataset)
    ...
    def extract_toPandas(self, dictVec):
        # Extract coefficients
        attribute_name = np.array(map(lambda st : st.rsplit('=', 1)[0], dictVec.get_feature_names()))
        attribute_value = np.array(map(lambda st : extract_feature_value(st), dictVec.get_feature_names()))
        ...
        for i_factor in range(self.num_factors):
            v_name = 'v' + str(i_factor)
            model_coef[v_name] = self.v[i_factor]

        return model_coef

    def extract_toJSON(self, dictVec):
        ...
        """
        model_coef = self.extract_toPandas(dictVec)
        columns_json = model_coef.columns[2:len(model_coef.columns)]

        output = {'intercept' : [self.w0]}
        for attr in np.unique(model_coef['Attribute_name']):
            model_coef_sub = model_coef[model_coef['Attribute_name'] == attr]
            output[attr] = {}
            for irow in range(model_coef.shape[0]):
                output[attr][model_coef['Attribute_value'][irow]] = []
                dict_empty = dict()
                for cols in columns_json:
                    dict_empty[cols] = model_coef[cols][irow]
                output[attr][model_coef['Attribute_value'][irow]].append(dict_empty)
        return output

def extract_feature_value(feature_string):
    feature_string_split = feature_string.rsplit('=', 1)
    if len(feature_string_split) > 1:
        return feature_string_split[1]
    else:
        return "NA"
